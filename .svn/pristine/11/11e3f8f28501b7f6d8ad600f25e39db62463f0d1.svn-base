...
// 加载配置文件
var objConfig = JSON.parse(fs.readFileSync(pathConfig.get_upLoadjs_json(), "utf8"));
//var objConfig = JSON.parse(fs.readFileSync("upLoadInfos/upLoadjs.json", "utf8"));
...
if(objConfig.appenders){
    ...
    {
        baseDir = pathConfig.get_debug_upload_logs_path();
    }else{
...
var deviceAttris = log4js.getLogger('deviceAttris');
var deviceStatus = log4js.getLogger('deviceStatus');
//var logWarn = log4js.getLogger('logWarn');
//var logErr = log4js.getLogger('logErr');

upLoadjs.writeAttris = function(msg)
{
    if(msg == null)
        msg = "";
    deviceAttris.info(msg);
};

upLoadjs.writeStatus = function(msg)
{
    if(msg == null)
        msg = "";
    deviceStatus.info(msg);
};

/*myLog4js.writeWarn = function(msg){
 if(msg == null)
 msg = "";
 logWarn.warn(msg);
 };

 myLog4js.writeErr = function(msg, exp){
 if(msg == null)
 msg = "";
 if(exp != null)
 msg += "\r\n" + exp;
...
/*exports.use = function(app) {
    ...
    app.use(log4js.connectLogger(deviceStatus, {level:'info', format:':method :url'}));
}*/
