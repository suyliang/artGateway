...
vows.describe('log4js fileAppender').addBatch({
  ...
  'with a max file size and no backups': {
    topic: function() {
      ...
      remove(testFile);
      remove(testFile + '.1');
      //logs file of 100 bytes maximum, no backups
      log4js.clearAppenders();
      log4js.addAppender(
        require('../lib/appenders/file').appender(testFile, log4js.layouts.basicLayout, 100, 0), 
        'max-file-size'
      );
      logger.info("This is the first logs message.");
      logger.info("This is an intermediate logs message.");
      logger.info("This is the second logs message.");
    ...
    'log file should only contain the second message': function(err, fileContents) {
      assert.include(fileContents, "This is the second logs message.");
      assert.equal(fileContents.indexOf("This is the first logs message."), -1);
    },
    'the number of files': {
      topic: function() {
      ...
      'starting with the test file name should be two': function(err, files) {
        //there will always be one backup if you've specified a max logs size
  ...
  'with a max file size and 2 backups': {
    topic: function() {
      ...
      //logs file of 50 bytes maximum, 2 backups
      log4js.clearAppenders();
      log4js.addAppender(
      ...
      logger.info("This is the first logs message.");
      logger.info("This is the second logs message.");
      logger.info("This is the third logs message.");
      logger.info("This is the fourth logs message.");
      var that = this;
    ...
    'the log files': {
      ...
      'and the contents of the first file': {
        ...
        'should be the last log message': function(contents) {
          assert.include(contents, 'This is the fourth logs message.');
        }
      },
      'and the contents of the second file': {
        ...
        'should be the third log message': function(contents) {
          assert.include(contents, 'This is the third logs message.');
        }
      },
      'and the contents of the third file': {
        ...
        'should be the second log message': function(contents) {
          assert.include(contents, 'This is the second logs message.');
        }
      }
    }
  },
  'with a max file size and 2 compressed backups': {
    ...
    'the log files': {
      ...
      'and the contents of the second file': {
        ...
        'should be the third log message': function(contents) {
          assert.include(contents.toString('utf8'), 'This is the third logs message.');
        }
      },
      'and the contents of the third file': {
        topic: function(logFiles) {
        ...
        'should be the second log message': function(contents) {
          assert.include(contents.toString('utf8'), 'This is the second logs message.');
        }
      }
    }
  }
  ...
  'configure' : {
    'with fileAppender': {
      topic: function() {
        ...
        , logger;
        //this config file defines one file appender (to ./tmp-tests.logs)
        //and sets the logs level for "tests" to WARN
