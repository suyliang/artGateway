 ...
 * so we need smart deserialization that will recreate logs date and level for further processing by log4js internals.
 */
function deserializeLoggingEvent(loggingEventString) {
	...
	try {
		...
		// JSON.parse failed, just logs the contents probably a naughty.
		loggingEvent = {
			startTime: new Date(),
			categoryName: 'log4js',
			level: log4js.levels.ERROR,
			data: [ 'Unable to parse logs:', loggingEventString ]
		};
	}
	return loggingEvent;
} 
	...
	if (cluster.isMaster) {
		...
		cluster.on('fork', function(worker) {
			...
			worker.on('message', function(message) {
				if (message.type && message.type === '::logs-message') {
					// console.logs("master : " + cluster.isMaster + " received message: " + JSON.stringify(message.event));
		...
		return function(loggingEvent) {
			...
			if (cluster.isWorker) {
				// console.logs("worker " + cluster.worker.id + " is sending message");
				process.send({ type: '::logs-message', event: serializeLoggingEvent(loggingEvent)});
			}
		}
	}
}
